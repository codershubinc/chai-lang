...
*   **`peekToken`**: The *next* token.
*   **Why two tokens?**: We often need to look ahead. For example, if we see `chai_say`, we need to know if the *next* token is a string to decide if it's valid.

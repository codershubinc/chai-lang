...
- **`position`**: The index of the character we are _currently_ looking at.
- **`readPosition`**: The index of the _next_ character. We need this to "peek" ahead (e.g., to see if `!` is followed by `=`).
...
- **How**: Loops `readChar()` until it hits another `"` or EOF. Returns the text _inside_ the quotes (excluding the quotes themselves).
